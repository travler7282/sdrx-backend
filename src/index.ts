...
app.get(['/healthz', '/sdrx/api/v1/healthz'], (_req: Request, res: Response) => {
  res.status(200).json({ status: 'ok' });
});

app.get(['/readyz', '/sdrx/api/v1/readyz'], (_req: Request, res: Response) => {
  res.status(200).json({ ready: true, service: 'sdr-express-app' });
});

app.get(['/api/sdr/status', '/sdrx/api/v1/status', '/sdrx/api/v1/api/sdr/status'], (_req: Request, res: Response) => {
  res.status(200).json(sdrState);
});

app.post(['/api/sdr/tune', '/sdrx/api/v1/tune', '/sdrx/api/v1/api/sdr/tune'], (req: Request, res: Response) => {
